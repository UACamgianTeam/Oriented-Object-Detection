...
def evaluate(data_path: str, test_images_dict: dict, desired_ids: set, 
    ...
    """ Evaluates the given images contained in test_images_dict using pycocotools.

    Evaluates per-image and per-window over each class category. The per-class-category AP
    result as well as the overall AP result are printed.
    """
...
def evaluate_set(data_path: str, test_images_dict: dict, desired_ids: set, 
    ...
    """ Evaluates the given set of images. The available sets are {'image', 'window'}.
    
    Evaluates using pycocotools per-class-category as well as overall. Results are printed.
    """
...
def write_image_results(results_path: str, test_images_dict: dict, min_threshold: float) -> None:
    """ Writes the results generated by the model (when running inference on the test set)
    to a results file in the coco format. This results file can then be compared to the
    ground truth coco file during evaluation.
...
def write_window_validation_file(window_validation_file_path: str, test_annotations: dict, test_images_dict: dict) -> None:
    ...
    This ground truth file does not exist prior to evaluation as opposed
    to the image validation file which should be provided in the dataset.
    """
    window_validation = {}
